...
public class ScraperService implements Scraper {
  ...
  public boolean scraper(final int page, final boolean test, final String gubun)
  ...
  private void hrdScraper(int page) throws IOException, InterruptedException {
    ...
    Document doc = Jsoup.connect(url)
        ...
        .header("Accept-Encoding", "gzip, deflate").data(params).timeout(60000)
        .post();
    ...
    for (Element e : elements) {
      ...
      input.setSeq(
          e.getAllElements().get(0).getElementsByTag("td").get(0).text());
      input.setEntrnm(
          e.getAllElements().get(0).getElementsByTag("td").get(1).text());
      input.setType(
          e.getAllElements().get(0).getElementsByTag("td").get(2).text());
      input.setBtpnm(
          e.getAllElements().get(0).getElementsByTag("td").get(3).text());
      input.setAddr(
          e.getAllElements().get(0).getElementsByTag("td").get(4).text());
      fileSave.saveFiles(InputHrd.class, input, outputDir, delimiter,
          "hrd1_out.txt");
  ...
  private void hrdScraper2(int page) throws IOException, InterruptedException {
    ...
    for (Element e : elements) {
      ...
      input.setBtpnm(
          e.getAllElements().get(0).getElementsByTag("td").get(4).text());
      input.setTel(
          e.getAllElements().get(0).getElementsByTag("td").get(5).text());
      input.setAddr(
          e.getAllElements().get(0).getElementsByTag("td").get(6).text());
      fileSave.saveFiles(InputHrd2.class, input, outputDir, delimiter,
          "hrd2_out.txt");
      log.info(input.toCsvForm(delimiter));
    }
    Thread.sleep(sleepCnt);
